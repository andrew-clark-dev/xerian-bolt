...
import { getSyncData } from "./graphql/queries";
import { updateSyncData } from "./graphql/mutations";
const client = generateClient();
export default function SyncDataUpdateForm(props) {
  const {
    id: idProp,
    syncData: syncDataModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = syncDataRecord
      ? { ...initialValues, ...syncDataRecord }
      : initialValues;
    setInterface1(cleanValues.interface);
    setTotal(cleanValues.total);
    setLastSync(cleanValues.lastSync);
    setErrors({});
  };
  const [syncDataRecord, setSyncDataRecord] = React.useState(syncDataModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getSyncData.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getSyncData
        : syncDataModelProp;
    ...
    queryData();
  }, [idProp, syncDataModelProp]);
  React.useEffect(resetStateValues, [syncDataRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateSyncData.replaceAll("__typename", ""),
            variables: {
              input: {
      ...
      {...getOverrideProps(overrides, "SyncDataUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Interface"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || syncDataModelProp)}
